...
class Forms {
    ...
    protected function generateFormFields() {
        $this->form_fields = [
            ...
            'question' => [
                'type' => 'textarea',
                'label' => 'What questions can we answer for you?',
                'errors' => [
                    'required' => 'Please enter any questions you have.',
                ],
                'include_in_email_template' => true,
            ],
            'mouthguard_comment' => [
            ...
            'event_select' => [
                ...
                'options' => $this->get_events_in_region(),
            ...
            'referring_doctor_name' => [
                ...
                'errors' => [
                    'required' => 'Please enter referring doctor name.',
                ],
                'include_in_email_template' => true,
            ],
            'referring_doctor_email' => [
                'type' => 'email',
                'label' => 'Referring doctor\'s email address',
                'errors' => [
                    'required' => 'Please enter your email address.',
                    'emailTLD' => 'Please enter a valid email address.',
                ],
            ...
            'patient_dob' => [
                'type' => 'text',
                'label' => 'Patient\'s D.O.B. (MM/DD/YYYY)',
            ...
            'referred_reason' => [
                'type' => 'checkbox-group',
                'class' => 'row-group-2',
                'label' => 'This patient is being referred for:',
                'options' => ['Full orthodontic evaluation', 'Pre-prosthetic treatment needed', 'Other'],
                'errors' => [
                    'required' => '',
                ],
                'include_in_email_template' => true,
    ...
    protected function generateFormConfigurations() {
        ...
        $this->form_configurations['questions'] = [
            ...
            'subject' => 'Free Consultations Questions - Form Submission',
            'recipients' => 'bbuisker@simspecialty.com',
            'cc' => 'kathleenf@mdgadvertising.com',
            'bcc' => 'webdept@mdgadvertising.com, bbuisker@simspecialty.com',
            'supports_email_notification' => true,
        ...
        $this->form_configurations['invisalign-virtual-care'] = [
            'cta' => 'Submit your request',
            'cta_class' => 'primary blue',
            'rows' => [
                ['full_name','email_address'],
            ...
            'subject' => 'Invisalign Virtual Care - Form Submission',
            'recipients' => 'bbuisker@simspecialty.com',
            'cc' => 'kathleenf@mdgadvertising.com',
            'bcc' => 'webdept@mdgadvertising.com, bbuisker@simspecialty.com',
            'supports_email_notification' => true,
        ...
        $this->form_configurations['event'] = [
            'cta' => 'Register for this event',
            'cta_class' => 'primary blue',
            'rows' => [
                ['event_select'],
                ['full_name', 'attendee_count'],
            ...
            'subject' => 'Confidence Counts - Event Form Submission',
            'recipients' => 'bbuisker@simspecialty.com',
            'cc' => 'kathleenf@mdgadvertising.com',
            'bcc' => 'webdept@mdgadvertising.com, bbuisker@simspecialty.com',
            'supports_email_notification' => true,
        ...
        $this->form_configurations['event']['hidden']['office_preference'] = empty($this->location) ? $this->brand->ID : $this->location->ID;
        ...
        $this->form_configurations['sleep-apnea-appointment'] = [
            'cta' => 'Send request',
            'cta_class' => 'primary blue',
            'rows' => [
                ['full_name','email_address'],
        ...
        $this->form_configurations['orthodontic-referral'] = [
            ...
            'cta_class' => 'primary blue',
            'rows' => [
                ['referring_practice_name','referring_doctor_name'],
                ['referring_doctor_email'],
                ['patient_name','patient_phone_number'],
                ['patient_dob','office_preference'],
                ['referred_reason'],
                ['check_all_that_apply'],
                ['panoramic_xray_title'],
                ['panoramic_xray_file'],
    ...
    private function getTemplateBody() {
        ...
        foreach($this->current_form->rows as $row) {
            foreach($row as $ff) {
                ...
                if(isset($field) && property_exists($field, 'include_in_email_template') && $field->include_in_email_template) {

                    if( $field->label == 'Event' ) {
                        $fields[$field->label] = !isset($data[$ff]) ? '' : get_event_name_by_id( $data[$ff] );
                    } else {
                        $fields[$field->label] = !isset($data[$ff]) ? '' : $data[$ff];
                    }
                }
            }
        }
    ...
    private function appendPostData() {
        ...
        // if ( $this->current_form_name == 'sleep-apnea-appointment' && is_location() && $brand->ID == 16618)
        // {
        // 	$_POST['recipient'] = get_email_addresses_for_form( $this->current_form_name, $location );
        // }
    ...
    private function captureLead() {
        ...
        $inserted = $wpdb->query($wpdb->prepare('insert into form_submissions(ID, form, ip_address, timestamp, metadata) values(NULL, %s, %s, %s, %s);',
            $this->current_form_name === 'consultation-no-virtual-consultation' ? 'consultation' : $this->current_form_name,
            $_POST['ip_address'],
            $_POST['timestamp_sql'],
    ...
    private function sendEmailNotification($attachments = false) {
        ...
        $recipient = 'info@kristoorthodontics.com';
        $recipient_by_form = get_email_addresses_for_form($this->current_form_name, !empty($_POST['event_select']) ? $_POST['event_select'] : $_POST['office_preference']);
    ...
    private function locationNameFromID($id) {
        $office_name = get_the_title($id);
        return strip_tags($office_name, "<span>");

        /*switch($id) {
            // Rapids Orthodontics
            case 9831: return 'Rapids Orthodontics';
            ...
            case 14242: return "Chapman Orthodontics";
            case 13035: return "Dietmeier Orthodontics";
            case 12153: return "Shawano Orthodontics";
    ...
    private function get_events_in_region() {
        $events_in_region = [];
        $events_for_select = [];
        ...
        if( is_location() ) {
            ...
            $events_in_region = get_events_for_region($region[0]->ID, true);
        }

        foreach ($events_in_region as $eir) {
            $events_for_select[$eir->ID] = $eir->post_title;
        }

        return $events_for_select;
    }
        ...
        add_action('admin_init', function() {
            ...
            if(isset($_GET['export'])) {
                ...
                $mapping = array(
                    ...
                    'events' => 'event', // form name is singular
                    'refer' => 'refer',
                    'community' => 'community',
