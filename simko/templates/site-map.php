...
if (empty($location)) {
	...
	if (!empty($provider_objects)) {
		...
		foreach ($provider_objects as $provider) {
			...
			$provider_links[] = '<li><a href="'.(brand_url("/".($provider->relative_url)."/")).'">'.($provider_title).'</a></li>';
		}
		$provider_links[] = '</ul>';
	}
	...
	if (!empty($location_objects)) {
		...
		if(is_single_location_brand()) {
			...
			$location_links[] = '<li><a href="'.(brand_url("/")).'">Orthodontist office in '.($location->post_title).', '.($location->state).' '.($location->zip).'</a></li>';
		} else {
			foreach ($location_objects as $location) {
				$location_links[] = '<li><a href="'.(brand_url("/".($location->relative_url)."/")).'">Orthodontist office in '.($location->post_title).', '.($location->state).' '.($location->zip).'</a></li>';
			}
		}
		$location_links[] = '</ul>';
	}
} else {
	array_push($exclusions, 'orthodontist-office');
...
if (!empty($page_slugs)) {
	...
	foreach ($page_slugs as $page) {
		...
		if (empty($page->post_parent) && !in_array($relative_exclusions, $exclusions)) {
			$page_links[] = '<li><a href="'.(brand_url($relative_url)).'">'.($page->post_title).'</a></li>';
		}
	}
	$page_links[] = '</ul>';
}
...
if (!empty($arrays_of_post_categories)) {
	...
	if (!empty($all_post_categories)) {
		...
		foreach ($all_post_categories as $category) {
			if(  strtolower( $category->name ) != 'hidden' ) {
				$category_links[] = '<li><a href="'.(brand_url('/orthodontic-blog/category/'.($category->slug).'/')).'">'.($category->name).'</a></li>';
			}
		}
		$category_links[] = '</ul>';
	}
