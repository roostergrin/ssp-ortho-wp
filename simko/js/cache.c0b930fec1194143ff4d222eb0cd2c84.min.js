...
$('span.upload').on('click',function(e){$(this).prev().trigger('click')})})(jQuery);(function($){let load_more=$('#load-more');load_more.data('loading',!1);load_more.on('click',function(e){e.preventDefault();load_more.html('Loading...');if(!load_more.data('loading')){load_more.data('loading',!0);$.post(ajax.url,{action:ajax.action,page:load_more.data('page'),posts_per_page:ajax.posts_per_page,post_type:ajax.post_type,e_id:ajax.e_id,t_id:ajax.t_id}).done(function(response){let data=JSON.parse(response);$('div.blog-posts .small-posts').append(data.html);if(data.more){load_more.data('page',load_more.data('page')+1)}else{load_more.remove()}
load_more.html('Load more');load_more.data('loading',!1)})}})})(jQuery);(function($){$(".btn.menu").click(function(){$(window).scrollTop(0);$('html').removeClass('contact-form');$("html").toggleClass("mobileMenu mobileNavigating");$("html").addClass("sticky-header");$('.btn.menu .hamburger--squeeze').toggleClass('is-active')});function onMobileNavSlideComplete(submenu){if($('#mobileNavigation .primary-nav ul.sub-menu.expanded').length>0){var top=submenu.parent().offset().top-$('.menu').offset().top;$('#mobile_navigation').animate({scrollTop:top},200,'linear')}
