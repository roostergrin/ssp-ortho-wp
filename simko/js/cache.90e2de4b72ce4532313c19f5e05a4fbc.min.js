...
function createSuccessFormSubmitDatalayerEvents(formData){if(!window.dataLayer)return;var eventModel={};var formName=formData.get('form_name');var locationSelectionOnlyForms=['questions','orthodontic-referral','refer','contact']
...
if($('form#form_questions').length){form_questions_recaptcha_widget=grecaptcha.render('google-recaptcha-container_questions',{sitekey:google_recaptcha_configuration.site_key,callback:onGoogleReCaptchaSubmit,size:'invisible',badge:'inline'})}
...
if($('form#form_invisalign-virtual-care').length){form_review_recaptcha_widget=grecaptcha.render('google-recaptcha-container_invisalign-virtual-care',{sitekey:google_recaptcha_configuration.site_key,callback:onGoogleReCaptchaSubmit,size:'invisible',badge:'inline'})}
if($('form#form_event').length){formEventRecaptchaWidget=grecaptcha.render('google-recaptcha-container_event',{sitekey:google_recaptcha_configuration.site_key,callback:onGoogleReCaptchaSubmit,size:'invisible',badge:'inline'})}}
...
function getCursorPos(e){var a,x=0;e=e||window.event;a=img.getBoundingClientRect();x=e.pageX-a.left;if(e.touches&&e.touches[0])x=e.touches[0].pageX-a.left;x=x-window.pageXOffset;return x}
function slide(x){img.style.width=x+"px";slider.style.left=img.offsetWidth-(slider.offsetWidth/2)+"px";sliderbar.style.left=img.offsetWidth-(sliderbar.offsetWidth/2)+"px"}}
function initComparisons(){var curSlider,curSliderbar,x,i;curSlider=document.getElementById("img-comp-slider");curSliderbar=document.getElementById("img-comp-slider-bar");if(curSlider!==null)curSlider.remove();if(curSliderbar!==null)curSliderbar.remove();x=document.getElementsByClassName("img-comp-overlay");for(i=0;i<x.length;i++){compareImages(x[i])}}
$(window).on('load resize',function(){initComparisons()});var smile_gallery_owl=$('.smile-gallery .owl-carousel');smile_gallery_owl.owlCarousel({items:1,slideBy:1,margin:10,nav:!0,navText:[],dots:!1,loop:!0,center:!0,autoWidth:!0,mouseDrag:!0,pullDrag:!0,freeDrag:!1,});smile_gallery_owl.on('translated.owl.carousel',function(e){$('ul.before-images li.img.active').removeClass('active');var pos=$('.smile-gallery .owl-item.active.center img').data('position');$('.smile-gallery ul.before-images img[data-position='+pos+']').parent().addClass('active');var container=$('.gallery .img-comp-container');var content='<div class="img-comp-img"><img src="'+$('.smile-gallery .owl-item.active.center img').data('after')+'" width="780" height="460" class="" alt="" /><p><strong>After</strong></p></div>';content+='<div class="img-comp-img img-comp-overlay"><img src="'+$('.smile-gallery .owl-item.active.center img').data('before')+'" width="780" height="460" class="" alt="" /><p><strong>Before</strong></p></div>';container.html(content);initComparisons()});$('ul.before-images li.img img').on('click',function(){$('ul.before-images li.img.active').removeClass('active');$(this).parent().addClass('active');$('.gallery .img-comp-container').empty().append($('<div class="img-comp-img">').append($('<img src="'+$(this).data('after')+'" width="780" height="460" alt="'+$(this).attr('alt').replace('Before','After')+'">'),$('<p>').html('<strong>After</strong>')),$('<div class="img-comp-img img-comp-overlay">').append($('<img src="'+$(this).data('before')+'" width="780" height="460" alt="'+$(this).attr('alt')+'">'),$('<p>').html('<strong>Before</strong>')),);initComparisons();var pos=$(this).data('position')-1;smile_gallery_owl.trigger('to.owl.carousel',[pos])})})(jQuery);(function($){let carousel='section.testimonials.carousel',log=!1,time=0,init=!0,paused=!1,autoplay=!0,autoplaying=!1,autoplay_speed=7000,autoplay_interval=!1;function update_height(){$(carousel).find('.content-carousel').css('height',$(carousel).find('.content-carousel .carousel-item.active > .content').outerHeight())}
function prev_slide(){if($(carousel).find('.content-carousel .carousel-item.active').prev('.carousel-item').length){$(carousel).find('.content-carousel .carousel-item.active').removeClass('active').prev('.carousel-item').addClass('active');$(carousel).find('.images-carousel .img-container.active').removeClass('active').prev('.img-container').addClass('active')}else{$(carousel).find('.content-carousel .carousel-item.active').removeClass('active');$(carousel).find('.images-carousel .img-container.active').removeClass('active');$(carousel).find('.content-carousel .carousel-item').last().addClass('active');$(carousel).find('.images-carousel .img-container').last().addClass('active')}
update_height()}
function next_slide(){if($(carousel).find('.content-carousel .carousel-item.active').next('.carousel-item').length){$(carousel).find('.content-carousel .carousel-item.active').removeClass('active').next('.carousel-item').addClass('active');$(carousel).find('.images-carousel .img-container.active').removeClass('active').next('.img-container').addClass('active')}else{$(carousel).find('.content-carousel .carousel-item.active').removeClass('active');$(carousel).find('.images-carousel .img-container.active').removeClass('active');$(carousel).find('.content-carousel .carousel-item:first').addClass('active');$(carousel).find('.images-carousel .img-container:first').addClass('active')}
update_height()}
function play(){paused=!1;if(!init)return!1;if($(carousel).find('.last-slide').length){$(carousel).find('.content-carousel .carousel-item.last-slide').removeClass('last-slide').addClass('active');$(carousel).find('.images-carousel .img-container.last-slide').removeClass('last-slide').addClass('active')}else{$(carousel).find('.content-carousel .carousel-item').first().addClass('active');$(carousel).find('.images-carousel .img-container').first().addClass('active')}
update_height();init=!1}
function stop(){$(carousel).find('.content-carousel .carousel-item.active').removeClass('active').addClass('last-slide');$(carousel).find('.images-carousel .img-container.active').removeClass('active').addClass('last-slide');update_height();init=!0;autoplaying=!1}
function pause(){paused=!0}
if($(carousel).length){$(carousel).find('.pagination-container .pagination .icon-left-arrow-thick').on('click',function(){prev_slide()});$(carousel).find('.pagination-container .pagination .icon-right-arrow-thick').on('click',function(){next_slide()});$(carousel).on('mouseenter',function(){pause()});$(carousel).on('mouseleave',function(){play()});$(carousel).on('click','a.read-more',function(e){e.preventDefault();$(this).parent().text($(this).parent().data('content'));update_height()});$(window).on('scroll resize',function(){if($(carousel+':inview').length){if(init)play();if(!autoplaying){autoplaying=!0;if(autoplay){autoplay_interval=setInterval(function(){if(!paused)next_slide()},autoplay_speed)}}}else{if(autoplaying){if(autoplay)clearInterval(autoplay_interval);autoplaying=!1}
if(!init)pause()}
update_height()})}})(jQuery);(function($){let carousel='section.overlap.carousel',device=$(window).width()>782?'desktop':'mobile',owl=$(carousel).find('.owl-carousel'),interval_desktop=null;function resetInterval(){if(interval_desktop)clearInterval(interval_desktop);interval_desktop=setInterval(function(){$(carousel).find('.pagination-container .pagination .page-right').trigger('click')},4000)}
if($(carousel).length){$(carousel).find('.pagination-container .pagination .page-left').on('click',function(){if($(carousel).find('aside .img-container.active').prev('.img-container').length){$(carousel).find('aside .img-container.active').removeClass('active').prev('.img-container').addClass('active')}else{$(carousel).find('aside .img-container.active').removeClass('active');$(carousel).find('aside .img-container').last().addClass('active')}
resetInterval()});$(carousel).find('.pagination-container .pagination .page-right').on('click',function(){if($(carousel).find('aside .img-container.active').next('.img-container').length>0){$(carousel).find('aside .img-container.active').removeClass('active').next('.img-container').addClass('active')}else{$(carousel).find('aside .img-container.active').removeClass('active');$(carousel).find('aside .img-container').first().addClass('active')}
resetInterval()});$(carousel).find('.mobile-pagination-container .mobile-pagination .icon-left-arrow-thick').on('click',function(){if($(carousel).find('article .images-carousel-mobile .img-container.active').prev('.img-container').length>0){$(carousel).find('article .images-carousel-mobile .img-container.active').removeClass('active').prev('.img-container').addClass('active')}else{$(carousel).find('article .images-carousel-mobile .img-container.active').removeClass('active');$(carousel).find('article .images-carousel-mobile .img-container').last().addClass('active')}});$(carousel).find('.mobile-pagination-container .mobile-pagination .icon-right-arrow-thick').on('click',function(){if($(carousel).find('article .images-carousel-mobile .img-container.active').next('.img-container').length>0){$(carousel).find('article .images-carousel-mobile .img-container.active').removeClass('active').next('.img-container').addClass('active')}else{$(carousel).find('article .images-carousel-mobile .img-container.active').removeClass('active');$(carousel).find('article .images-carousel-mobile .img-container').first().addClass('active')}});$(window).on('scroll load resize',function(){if($(carousel).find('.main-container:inview').length>0){$(carousel).find('article .carousel-item.active').removeClass('active');$(carousel).find('aside .img-container.active').removeClass('active');$(carousel).find('article .carousel-item').first().addClass('active');$(carousel).find('aside .img-container').first().addClass('active');if(interval_desktop===null)resetInterval()}else{$(carousel).find('article .carousel-item.active').first().removeClass('active');$(carousel).find('aside .img-container.active').first().removeClass('active');if(interval_desktop){clearInterval(interval_desktop);interval_desktop=null}}
if($(window).width()>782&&device!='desktop'){if($(owl).hasClass('owl-carousel')){}}else if($(window).width()<783&&device!='mobile'){if(!$(owl).hasClass('owl-carousel')){}}
device=$(window).width()>782?'desktop':'mobile'})}
$(owl).owlCarousel({items:1,slideBy:1,margin:10,loop:!0,dots:!1,nav:!1,mouseDrag:!0,touchDrag:!0,autoplay:!1,autoplayHoverPause:!0});$(owl).on('changed.owl.carousel',function(e){$(owl).trigger('stop.owl.autoplay');$(owl).trigger('play.owl.autoplay')});$(carousel).find('.mobile-pagination .icon-left-arrow-thick, .page-left').click(function(){$(owl).trigger('prev.owl.carousel')});$(carousel).find('.mobile-pagination .icon-right-arrow-thick, .page-right').click(function(){$(owl).trigger('next.owl.carousel')})})(jQuery)