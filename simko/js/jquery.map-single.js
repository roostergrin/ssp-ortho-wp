(function($) {
	...
	// Omit telephone from Single Location Brands
	const noPhoneLinks = ['Rapids Orthodontics', 'Shawano Orthodontics', 'Dietmeier Orthodontics', 'Chapman Orthodontics']
	var mapPhoneLink = !noPhoneLinks.includes(map_data.name) ? '<p><a href="tel:+1'+map_data.phone_number+'" class="white">'+map_data.phone_number.format_phone('dot')+'</a></p>' : "";
	var info_window = new InfoBox({
		content: '<div class="info-window"><div class="slide"><p>' + map_data.name + '</p>' + mapPhoneLink + '<p><a href="' + map_data.directions + '" class="cta text white" target="_blank">Directions</a></p><a class="close-info-box" href="javascript:close_info_box()"><i class="icon-close"></i></a></div></div>',
		disableAutoPan: true,
		maxWidth: 400,
