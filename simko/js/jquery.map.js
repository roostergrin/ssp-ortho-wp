(function($) {
	...
	function init_maps() {
		...
		map_object.addListener('zoom_changed', function() {
			if (current_zoom != map_object.getZoom()) {
				...
				google.maps.event.addListenerOnce(map_object, 'idle', function() {
					...
					// pan map a little for Ross Ortho
					if(map_data.brand && map_data.brand == 'smiles-by-ross'){
						map_object.panBy( map_object.panBy_x, map_object.panBy_y );
					}
				});
			}
		...
		if(map_data.brand && map_data.brand == 'smiles-by-ross') {
			map_object.panBy_x = -200;
			map_object.panBy_y = 0;
		}		
