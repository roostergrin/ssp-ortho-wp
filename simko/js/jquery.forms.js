(function($) {
	...
	function handleEventsFormSubmission() {
		...
		if(totalEventsRegisteredFor < totalEventCount) {
			...
			$formWrapper.empty().append(
				$('<p style="'+_styles+'">').text(text),
				$registerAgainButton
			);

			// center submission text/button
			...
			$registerAgainButton.on('click', function(evt) {
				...
				$formWrapperChildElements.each(function(i, el){
					if($(el).is('form')) {
						...
						$form.find('input[type="submit"]').prop('disabled', false).val('Register for this event');
				...
				// re-set up fancy-select click event
				...
				// Events page select populate hidden input
	...
	// Fancy Select Code...
	{
		// Preselect the first option in fancy containers if only one item exists
		...
		$('#fancy-select ul li').on('click', function(e) {
			...
			// if this event wasn't associated with the Events form...
	...
	window.onGoogleReCaptchaLoad = function() {
		...
		if($('form#form_questions').length) {
			form_questions_recaptcha_widget = grecaptcha.render('google-recaptcha-container_questions', {
				sitekey: google_recaptcha_configuration.site_key,
				callback: onGoogleReCaptchaSubmit,
				size: 'invisible',
				badge: 'inline'
		...
		if($('form#form_invisalign-virtual-care').length) {
			form_review_recaptcha_widget = grecaptcha.render('google-recaptcha-container_invisalign-virtual-care', {
				sitekey: google_recaptcha_configuration.site_key,
				callback: onGoogleReCaptchaSubmit,
				size: 'invisible',
				badge: 'inline'
		...
		if($('form#form_event').length) {
			formEventRecaptchaWidget = grecaptcha.render('google-recaptcha-container_event', {
				sitekey: google_recaptcha_configuration.site_key,
				callback: onGoogleReCaptchaSubmit,
				size: 'invisible',
				badge: 'inline'
