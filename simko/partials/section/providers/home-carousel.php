...
wp_enqueue_script('internal-providers-tri-carousel');
?>
<section class="providers home-carousel">
	<? if (!empty($providers)) : ?>
					...
					<? foreach ($providers as $provider) : ?>
						<div class="img-container">
							<? if (!empty($provider->image)) : ?>
								<img<?= !empty($provider->image['src']) ? ' src="'.($provider->image['src']).'"' : ''; ?><?= !empty($provider->image['width']) ? ' width="'.$provider->image['width'].'"' : ''; ?><?= !empty($provider->image['height']) ? ' height="'.$provider->image['height'].'"' : ''; ?><?= !empty($provider->image['data-label']) ? ' data-label="'.$provider->image['data-label'].'"' : ''; ?><?= !empty($provider->image['alt']) ? ' alt="'.$provider->image['alt'].'"' : ''; ?><?= !empty($provider->image['classes']) ? ' class="'.implode(' ', $provider->image['classes']).'"' : ''; ?> loading="lazy" />
							<? endif; ?>
							<? if (!empty($provider->mobile_image)) : ?>
								<img<?= !empty($provider->mobile_image['src']) ? ' src="'.($provider->mobile_image['src']).'"' : ''; ?><?= !empty($provider->mobile_image['width']) ? ' width="'.$provider->mobile_image['width'].'"' : ''; ?><?= !empty($provider->mobile_image['height']) ? ' height="'.$provider->mobile_image['height'].'"' : ''; ?><?= !empty($provider->mobile_image['data-label']) ? ' data-label="'.$provider->mobile_image['data-label'].'"' : ''; ?><?= !empty($provider->mobile_image['alt']) ? ' alt="'.$provider->mobile_image['alt'].'"' : ''; ?><?= !empty($provider->mobile_image['classes']) ? ' class="'.implode(' ', $provider->mobile_image['classes']).'"' : ''; ?> loading="lazy" />
							<? endif; ?>
							<div class="overlay">
								<p class="name"><a href="<?= brand_url('/'.($provider->relative_url).'/'); ?>"><?= $provider->caption['name']; ?></a></p>
