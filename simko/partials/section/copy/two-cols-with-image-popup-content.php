        ...
        <div class="inner-content">
            ...
            <aside>
                ...
                <div class="img-container desktop">
                    <? if (!empty($image)) : ?>
                        <img<?= !empty($image['style_desktop']) ? ' style="'.$image['style_desktop'].'"' : ''; ?><?= !empty($image['src']) ? ' src="'.(brand_host().$image['src']).'"' : ''; ?><?= !empty($image['width']) ? ' width="'.$image['width'].'"' : ''; ?><?= !empty($image['height']) ? ' height="'.$image['height'].'"' : ''; ?><?= !empty($image['srcset']) ? ' srcset="'.$image['srcset'].'"' : ''; ?><?= !empty($image['sizes']) ? ' sizes="'.$image['sizes'].'"' : ''; ?><?= !empty($image['alt']) ? ' alt="'.$image['alt'].'"' : ''; ?><?= !empty($image['classes']) ? ' class="'.str_replace('bg-img ', '', implode(' ', $image['classes'])).'"' : ''; ?> />
                    <? endif; ?>
                </div>
                <div class="img-container mobile">
                    <? if (!empty($mobile_image)) : ?>
                        <img<?= !empty($image['style_mobile']) ? ' style="'.$image['style_mobile'].'"' : ''; ?><?= !empty($mobile_image['src']) ? ' src="'.(brand_host().$mobile_image['src']).'"' : ''; ?><?= !empty($mobile_image['width']) ? ' width="'.$mobile_image['width'].'"' : ''; ?><?= !empty($mobile_image['height']) ? ' height="'.$mobile_image['height'].'"' : ''; ?><?= !empty($mobile_image['srcset']) ? ' srcset="'.$mobile_image['srcset'].'"' : ''; ?><?= !empty($mobile_image['sizes']) ? ' sizes="'.$mobile_image['sizes'].'"' : ''; ?><?= !empty($mobile_image['alt']) ? ' alt="'.$mobile_image['alt'].'"' : ''; ?><?= !empty($mobile_image['classes']) ? ' class="'.str_replace('bg-img ', '', implode(' ', $mobile_image['classes'])).'"' : ''; ?> />
                    <? endif; ?>
                </div>
