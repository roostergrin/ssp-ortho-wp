...
?>
<form action="<?= brand_url($action ?? '#form_'.$form_name); ?>" method="POST" id="form_<?= $form_name; ?>" class="form_<?= $form_name; ?><? if($form_name == 'step'): ?> contact-form-scroll<? endif; ?><? if($network_error): ?> network-error<? endif; ?>" autocomplete="on" novalidate="novalidate" enctype="multipart/form-data; charset=utf-8; boundary=<?= rand(2, 1000); ?>">
	...
	<div class="row row-<?= $i; ?><?= (!empty($row[0] == 'panoramic_xray_file') || !empty($row[0] == 'event_flyer_file')) ? ' '.($row[0]) : ''; ?>">
		...
		?>
			<div class="row-item row-<?= $name; ?><?= (!empty($field->class)) ? ' '.($field->class) : ''; ?>">
				<? if($name == 'cta'): ?>
					<input type="submit" class="cta primary large fill form-submit" value="<?= $cta; ?>" /></div>
				<? continue; endif; ?>
				...
				<? elseif($field->type == 'fancy-select' && $form_name != 'event'):
					$offices = get_locations_for_brand(is_brand()->ID);
					if(get_the_id() === 6704) { // Great River - Landing Page - Free Consultation Minnesota
						$offices = array_filter($offices, function($var) {
							return $var->state === 'MN';
						});
					} elseif(get_the_id() === 6706) { // Great River - Landing Page - Free Consultation Wisconsin
						$offices = array_filter($offices, function($var) {
							return $var->state === 'WI';
						});
					} elseif(get_the_id() === 8659) { // Kristo - Free Custom Mouthguard page
						$offices = array_filter($offices, function($office) {
							return in_array($office->ID, [1247, 1249, 1251, 1253, 1257, 1265, 1269, 1271]);
						});
					} elseif(get_the_id() === 10783) { // Kristo - Free Custom Mouthguard v2 page
						$offices = array_filter($offices, function($office) {
							return in_array($office->ID, [1259, 1271]);
						});
					} elseif( get_page_template_slug() === 'templates/invisalign-virtual-care.php') {
						$newOffices = array();
						foreach ($offices as $location) {
							if(!empty($location->invisalign_vc_toggle) || $location->invisalign_vc_toggle == 1 ) {
								$newOffices[] = $location;
							}
						}
						$offices = $newOffices;
				...
				<? elseif($field->type == 'fancy-select' && $form_name == 'event'): ?>
					<div class="fancy-select" id="fancy-select">
						<? if (!empty($field->options)): ?>
							<div class="fancy-select-title">Event</div>
							<div class="options hidden">
								<ul class="select-options">
