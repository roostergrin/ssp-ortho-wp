...
<html <? language_attributes(); echo $html_classes; ?>>
	<head>
		...
		<meta http-equiv="Content-Security-Policy" content="default-src 'self' data: gap: https://ssl.gstatic.com 'unsafe-eval'; style-src-elem 'self' 'unsafe-inline'; media-src *; script-src 'self' *.jotfor.ms widgets.jotform.io 'unsafe-inline' 'unsafe-eval';">
		...
		<?php if( $brand->ID === 8643 || $brand->ID === 3291 || $brand->ID === 13032 || $brand->ID === 16332 || $brand->ID === 16618 || $brand->ID === 12097 || $brand->ID === 13590): ?>
			<script 
